# TODO read all raw images from files and perform filering stages:
 # -- using OCV remove small nonzero clusrters
# drop noisy (blurry) images from dataset at all
# align color of remaining images somehow (# TODO --- how it works?)
...
def make_masks_from_countures(depth_images_filenames):
    depth_images = [cv2.imread(fname, cv2.CV_8UC1) for fname in depth_images_filenames]
    depth_preprocessed = [ cv2.GaussianBlur(im, (5, 5), 5) for im in depth_images ]

    # TODO use advanced thresholding https://docs.opencv.org/master/d7/d4d/tutorial_py_thresholding.html#gsc.tab=0
    binary_masks = [ (cv2.threshold(d, 0, 255, cv2.THRESH_OTSU))[1] for d in depth_preprocessed]

    countours = [
        (cv2.findContours(bm, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE))[0] for bm in binary_masks
    ]
    largest_contour = [
        max(contours_list, key=lambda x: cv2.contourArea(x))
        for contours_list in countours
    ]
    masks = [ np.zeros_like(dp_im) for dp_im in depth_images]
...
def color_based_clustering_masks(image_filename, color_clustering_config, debug_mode = False):
    ...
    filtered_controus = [ contour for contour in contours if cv2.contourArea(contour) > cont_len_thresh ]
    ...
    if debug_mode:
        ...
        print(biggest_cluster_index)
        print(biggest_cluster_n_elements)
        ...
        cv2.imshow("image", np.hstack([image,
                                       ...
                                       cv2.cvtColor(masking_filtered, cv2.COLOR_GRAY2BGR)
                                       ]))
        cv2.waitKey(0)
    return masking_filtered


DEFAULT_CONFIFG_FILENAME = 'cfg/reconstruction/default/filter_outlier_raw_images.json'

def get_dataset_files(config):
...
if __name__ == "__main__":
    ...
    depth_image_path = get_file_list(input_depth_path,
                                     extension=".png")
    color_image_path = get_file_list(input_color_path,
                                     extension=".jpg")
    ...
    masks = []
    if config['use_color_cluster_thresholds']:
        ...
        if config["python_multi_threading"]:
            ...
            results = Parallel(n_jobs=MAX_THREAD)(delayed(
                color_based_clustering_masks)(col_im_path, color_clustering_config)
                                                  for col_im_path in tqdm(color_image_path) )
    ...
    elif config['blobs_len_thresholding']:  # this clustering approach uses length of cluster border as the basis for filtering
        print("blobs_len_thresholding")
        masks = make_masks_from_countures_for_loop(depth_image_path)


    print("End computing masks {}".format(time() - start_time))
    depth_images = [cv2.imread(fname, cv2.IMREAD_ANYDEPTH) for fname in depth_image_path]
    color_images = [cv2.imread(fname,cv2.COLOR_BGR2RGB) for fname in color_image_path]
